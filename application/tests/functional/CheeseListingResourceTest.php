...
class CheeseListingResourceTest extends CustomApiTestCase
{
    ...
    public function testPublishCheeseListingValidation()
    {
        ...
        $this->login($client, $adminUser->getEmail(), UserFactory::DEFAULT_PASSWORD);
        $client->request('PUT', '/api/cheeses/'.$cheeseListing->getId(), [
            'json' => ['isPublished' => true],
        ]);
        ...
        $this->login($client, $user->getEmail(), UserFactory::DEFAULT_PASSWORD);
        $client->request(
            'PUT', '/api/cheeses/'.$cheeseListing->getId(), [
            'json' => ['price' => 12345],
        ]);
        ...
        $this->assertResponseStatusCodeSame(403, 'normal user cannot unpublish');
